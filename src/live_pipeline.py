...
@dataclass(frozen=True)
class LiveRunResult:
    contract_address:str; pipeline:PipelineResult|None; notified:bool; error:str|None=None; wallet_score:float|None=None; alert_type:str|None=None
    @property
...
class LiveScannerRunner:
    ...
    def _watch(self,candidate,reason,**extra):
        t=candidate.token; self.watchlist_store.upsert(t.address,symbol=t.symbol,name=t.name,market_cap_usd=t.market_cap_usd,liquidity_usd=t.liquidity_usd,volume_24h_usd=t.volume_24h_usd,token_age_hours=t.token_age_hours,rejection_reason=reason,**extra)
    def run_once(self):
        results=[]
        for candidate in self.collector.collect():
            ...
            try:
                ...
                if payload is not None and self.transport is not None:
                    ...
                    if not self._recently_notified(mint,now,kind or "BUY",cooldown): self.transport.send(Alert(text=payload.text,contract_address=payload.contract_address)); notified=True
                    else: payload=None
                d=pr.decision
                if d.breakdown is None: raise RuntimeError("Decision result did not contain a score breakdown")
                if payload is None: self._watch(candidate,d.reasons[0] if d.reasons else d.decision.value,decision=d.decision.value,score=d.score,confidence=d.confidence,risk_overall=evidence.risk.overall_risk,utility_verified=evidence.utility.verified,evidence_urls=list(evidence.utility.evidence_urls))
                else: self.watchlist_store.remove(mint)
                self.outcome_store.append(AlertOutcomeRecord.from_decision(event_id=mint+":"+candidate.token.observed_at.isoformat(),token=candidate.token,decision=d.decision,score=d.breakdown,confidence=d.confidence,risk_overall=evidence.risk.overall_risk,risk_hard_filter_failed=evidence.risk.hard_filter_failed,why_now=why_now,invalidation_conditions=evidence.invalidation_conditions,wallet_intelligence_score=wallet.actionable_score,notified=notified,observed_at=candidate.token.observed_at,alert_type=kind or "BUY"))
                results.append(LiveRunResult(mint,pr,notified,wallet_score=wallet.actionable_score,alert_type=kind))
            except Exception as exc:
                logger.warning("Candidate %s skipped: %s",mint,exc); self._watch(candidate,str(exc),decision="EVIDENCE_PENDING"); results.append(LiveRunResult(mint,None,False,error=str(exc)))
        return results
