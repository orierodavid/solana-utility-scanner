...
class TokenMarketData(BaseModel):
    ...
    market_cap_usd:float=Field(ge=0); liquidity_usd:float=Field(ge=0); volume_24h_usd:float=Field(ge=0); volume_1h_usd:Optional[float]=Field(default=None,ge=0); price_usd:float=Field(ge=0)
    holders:Optional[int]=Field(default=None,ge=0); holder_growth_24h_pct:Optional[float]=None; buy_count_24h:Optional[int]=Field(default=None,ge=0); sell_count_24h:Optional[int]=Field(default=None,ge=0); buy_count_1h:Optional[int]=Field(default=None,ge=0); sell_count_1h:Optional[int]=Field(default=None,ge=0); volume_change_24h_pct:Optional[float]=None; price_change_24h_pct:Optional[float]=None; price_change_1h_pct:Optional[float]=None; price_change_5m_pct:Optional[float]=None; token_age_hours:Optional[float]=Field(default=None,ge=0); top_holder_concentration_pct:Optional[float]=Field(default=None,ge=0,le=100); creator_holding_pct:Optional[float]=Field(default=None,ge=0,le=100); mint_authority_active:Optional[bool]=None; freeze_authority_active:Optional[bool]=None; observed_at:datetime=Field(default_factory=lambda:datetime.now(timezone.utc))
    @field_validator("address")
    @classmethod
    def validate_solana_mint_address(cls,value:str)->str:
        if not _SOLANA_BASE58.fullmatch(value):raise ValueError("address must be a valid-looking Solana Base58 mint address")
        return value
    @field_validator("chain")
    @classmethod
    def solana_only(cls,value:str)->str:
        if value.lower()!="solana":raise ValueError("Only Solana tokens are supported")
        return "solana"
    @model_validator(mode="after")
    def validate_market_data(self)->"TokenMarketData":
        if self.market_cap_usd>0 and self.liquidity_usd>self.market_cap_usd*10:raise ValueError("Liquidity is implausibly high relative to market cap")
        if self.volume_1h_usd is not None and self.volume_1h_usd>self.volume_24h_usd*1.05:raise ValueError("1h volume cannot materially exceed 24h volume")
        return self
    @property
...
class TokenAnalysis(BaseModel):
    ...
    @property
    def contract_address(self)->str:return self.token.address
    @model_validator(mode="after")
