...
class WebEvidenceClient:
    """Read-only HTTP client with strict response limits."""
    ...
    def fetch(self, url: str) -> EvidenceDocument | None:
        ...
        try:
            response = self.session.get(
                url,
                headers={"User-Agent": "solana-utility-scanner/1.0", "Accept": "text/html,application/xhtml+xml"},
...
class GitHubEvidenceClient:
    """Read-only GitHub API activity lookup."""
    ...
    def repository_activity(self, repo_url: str) -> tuple[str, int, int]:
        ...
        api_url = f"{self.API}/repos/{owner}/{repo}"
        ...
        try:
            ...
            commits = self.session.get(
                f"{api_url}/commits", headers=headers,
                params={"since": since.isoformat().replace("+00:00", "Z"), "per_page": 20}, timeout=self.timeout,
            )
            issues = self.session.get(
                f"{api_url}/issues", headers=headers,
                params={"state": "all", "since": since.isoformat().replace("+00:00", "Z"), "per_page": 20}, timeout=self.timeout,
            )
            commit_payload = commits.json() if commits.ok else []
            issue_payload = issues.json() if issues.ok else []
            return (
                f"https://github.com/{owner}/{repo}",
                len(commit_payload) if isinstance(commit_payload, list) else 0,
                len(issue_payload) if isinstance(issue_payload, list) else 0,
            )
...
def _extract_urls(profile: Mapping[str, Any]) -> list[str]:
    ...
    for value in candidates:
        ...
        if parsed.netloc and value not in seen:
            seen.add(value)
            result.append(value)
    return result[:6]
...
def _risk_from_security(candidate: CollectedToken) -> RiskAssessment:
    ...
    if token.top_holder_concentration_pct is None:
        holder_risk = 70
        reasons.append("Top-holder concentration could not be independently verified")
    elif token.top_holder_concentration_pct > 35:
        holder_risk = 90
        reasons.append(f"Top-holder concentration is {token.top_holder_concentration_pct:.1f}%")
    elif token.top_holder_concentration_pct > 25:
        holder_risk = 60
        reasons.append(f"Top-holder concentration is elevated at {token.top_holder_concentration_pct:.1f}%")
    elif token.top_holder_concentration_pct > 15:
        holder_risk = 30
    ...
    if ratio < 0.05:
        liquidity_risk = 85
        reasons.append("Liquidity is very thin relative to market cap")
    elif ratio < 0.10:
        liquidity_risk = 60
        reasons.append("Liquidity is thin relative to market cap")
    elif ratio < 0.20:
        liquidity_risk = 35

    contract_risk = 0
    if token.mint_authority_active is None or token.freeze_authority_active is None:
        contract_risk += 30
        reasons.append("Token authority state is incomplete")
    else:
    ...
    if security is None:
        rug_risk = 80
        creator_risk = 70
        reasons.append("Independent security report is unavailable")
    else:
        raw = security.raw
        risk_items = raw.get("risks") if isinstance(raw, Mapping) else None
        if isinstance(risk_items, list):
            ...
            if rug_risk:
                reasons.append("Independent security data contains elevated risk findings")
    ...
    return RiskAssessment(
        ...
        contract_risk=min(contract_risk, 100),
...
class LiveEvidenceProvider:
    """Resolve live candidates into source-backed utility, catalyst and risk evidence."""

    def __init__(
        self,
        web: WebEvidenceClient | None = None,
        github: GitHubEvidenceClient | None = None,
        *,
        max_documents: int = 4,
    ) -> None:
        self.web = web or WebEvidenceClient()
        self.github = github or GitHubEvidenceClient()
    ...
    def enrich(self, candidate: CollectedToken):
        ...
        token = candidate.token
        urls = _extract_urls(candidate.profile)
        ...
        symbol_mentions = len(re.findall(
            rf"(?<![A-Za-z0-9]){re.escape(token.symbol.lower())}(?![A-Za-z0-9])", combined.lower()
        ))
        ...
        for url in urls:
            if "github.com/" in url.lower():
                repo, commits, issues = self.github.repository_activity(url)
        ...
        utility = UtilityEvidence(
            ...
            token_is_used_by_product=token_is_used,
        ...
        invalidations = (
            "Liquidity falls materially below the current screening level.",
            "Top-holder concentration rises above the hard risk threshold.",
            "Security evidence identifies a material rug, authority, or wallet risk.",
            "Momentum or volume deteriorates enough to invalidate the current setup.",
        )
        return CandidateEvidence(
            utility=utility,
            risk=risk,
            why_now=finding.thesis,
            catalyst_score=catalyst_score,
            confidence=min(evidence_confidence, finding.confidence),
