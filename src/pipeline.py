...
from .decision import DecisionAlertPipeline if False else DecisionResult
...
class DecisionAlertPipeline:
    ...
    def _build_early_buy_alert(
        ...
        *,
        ...
        This path intentionally does not call the legacy BUY-only alert builder;
        EARLY_BUY is an actionable entry state in the current strategy.
        """
        ...
        *,
        ...
        if self.decision_engine.is_alertable(decision) and why_now.strip():
            if decision.decision is Decision.EARLY_BUY:
                alert = self._build_early_buy_alert(
                    token,
                    utility,
                    risk,
                    decision,
