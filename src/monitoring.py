...
def build_scan_health(started_at:datetime,finished_at:datetime,results:list[Any])->ScanHealth:
    ...
    return ScanHealth(started_at.astimezone(timezone.utc),finished_at.astimezone(timezone.utc),len(results),sum(r.pipeline is not None for r in results),sum(r.should_notify for r in results),sum(r.notified for r in results),sum(r.error is not None and r.delivery_error is None for r in results),sum(r.delivery_error is not None for r in results),sum(getattr(r,"re_evaluated",False) for r in results),duration)
